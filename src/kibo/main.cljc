(ns kibo.main
  "Kotodama WASM entrypoint for the Kibo clean-room actor (L4) — Clojure port.
  ...
  facts (`kibo.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"kibo.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "kibo-compat" :tier tier :entities entities} 200])
